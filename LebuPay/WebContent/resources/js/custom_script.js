//on click button
function nextDiv(openId, closeId){
  $(openId).show();
  $(closeId).hide();
}
...
$('input[type=file].customFile').customFile();

//custom choosen link end

//browse button
$(document).on('click', '.browse', function(){
	  var file = $('#fileUp').find('.newfile');
	  file.trigger('click');
	});
	$(document).on('change', '.newfile', function(){
		$('#fileUp').find('.file-upload-input').val($(this).val().replace(/C:\\fakepath\\/i, ''));
	});


//color picker area script
if($('#background').length > 0){
  var val = document.querySelector('#background'),
      box = document.createElement('div'),
      outer = document.createElement('span');
      outer.appendChild(box);
  box.className = 'color-box';
  outer.className = 'colorArea';
  box.style.backgroundColor = val.value;
  box.setAttribute('data-color', val.value);
  val.parentNode.insertBefore(outer, val);
  val.type = 'show';
