...
function isValidateMobileNumber(mobileNo) {
...
$( document ).ready(function() {
	...
	$( "form" ).submit(function( event ) {
		  ...
		  if(url != "merchant/complete-validation"){ 
		  ...
		  if(url.toLowerCase().indexOf("merchant") != -1){
			  ...
			  if(url == "merchant/login"){
				  ...
				  successResponse = function(response, messageSecdom){
					  ...
					  $('input[name="csrfPreventionSalt"]').val(response.csrfPreventionSalt);
					  messageSecdom.closest("form").find('#captcha_image').attr("src", "./ImageCreator?aa="+Math.random());
				  }; 
			  }
				  ...
				  $('#merchant_forget_password_modal').modal('hide');
				  successResponse = function(response, messageSecdom){
					  // messageSecdom.hide();
					  if(response.status != "ACTIVE"){
						  jQuery.growl.error({
		  ...
		  if(isValied){
			 /* if(form.find("input[type='password']").length != 0){
			  ...
			  $.each(submissionInfos, function( index, value ) {
				  ...
				  if(value.name == "mobileNo" && form.find("#selectCountry").length != 0){
				  ...
				  else if(value.name == "userName" && ajaxData[value.name].indexOf("@") == -1 && (url == "merchant/login" || url == "customer/login")){
					  /**
					   * At the time of login +880 will statically concat with mobile no
					   */
					  ajaxData[value.name] =  "+880"+ajaxData[value.name]
				  }
			  });
			  ...
			  sendAjax(ajaxConfig, successResponse, $(this));
		  }
	...
	$("#complete-validation-form").submit(function(){
		var email = $.trim($("#completeVerificationEmailId").val());
		var phoneCode = $.trim($("#completeVerificationPhoneCode").val());
