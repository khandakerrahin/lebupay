;(function($){
    ...
    var sys = {
        ...
        template:'<div id="{{id}}" class="el-notify-box {{className}} {{direction}}">{{iconHTML}}<div class="{{className}}-group"><h2 class="{{className}}-title">{{title}}</h2><div class="{{className}}-content"><p>{{content}}</p></div>{{closeHTML}}</div></div>',
