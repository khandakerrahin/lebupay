			...
			<div class="adminformBox tableWrap bordergrey">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table id="query" class="table" style="width: 100%;">
							<thead>
								<tr>
									<th>QUERY ID</th>
									<th>NAME</th>
									...
									<th>QUERY REPLY</th>
									<th>STATUS</th>
									<th class="datatableLinkButton">REPLY</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="contactUsModel" items="${contactUsModels }"
									varStatus="loop">
									<tr class="${((loop.index +1)%2)==0?'odd':'even'}">
										<td>${contactUsModel.contactUsId }</td>
										<td>${contactUsModel.name }</td>
										<td>${contactUsModel.emailId }</td>
										<td>${contactUsModel.subject }</td>
										<td>${contactUsModel.reply }</td>
										<td>${contactUsModel.status }</td>
										<td><a href="" class="edit" data-url="reply-contactus"
											data-target="contactUsId"><i class="fa fa-pencil-square-o"></i></a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Delete Net Querying</h4>
			</div>
			<form>
				<div class="modal-body">
					<input type="hidden" />
					<div id="message">You want to delete this Net Querying?</div>
				</div>
...
<%
					message = (String) request.getAttribute("query.name.required");
						if (message != null) {
					%>
<script>
 $(document).ready(function() {
...
<%
					message = (String) request.getAttribute("common.reply");
						if (message != null) {
					%>
<script>
 $(document).ready(function() {
...
<script type="text/javascript">
	$(document).ready(function() {
		var urlConfigObj = {"noOfPages":2, "url": "list-contactus", "method":"POST"};
		// it can autometicall set mobileNumber to mobile_number if other mapping is there then have to mention
        var columnsVSTable = {"contactUsId":"CONTACTUS_ID","emailId":"EMAILID"};
        var columns = [
		 { "data": "contactUsId", "defaultContent": "", "visible": false },
         ...
         { "data": "status", "defaultContent": "" },
         {
        	 "data": null,
             className: "center",
             defaultContent: '<a href="" class="edit" data-url="reply-contactus" data-target="contactUsId"><i class="fa fa-pencil-square-o"></i></a>'
         }
        ];
        ...
        drawDataTable($("#query"), urlConfigObj, columns, columnsVSTable, true, null, null, ${totalSize }, "downloadContactUsListExcel", changeActiveDeactive, 'rtp', $(".mySearch"), $(".myExcelDownload"), $(".mySelectOpt"));
