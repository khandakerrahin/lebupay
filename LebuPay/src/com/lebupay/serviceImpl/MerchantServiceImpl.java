...
/**
 * This is MerchantServiceImpl Class implements MerchantService interface is
...
public class MerchantServiceImpl extends BaseDao implements MerchantService {
	...
	public long create(MerchantModel merchantModel, HttpServletRequest httpServletRequest) throws Exception {
		...
		try {
			...
			String header = "OTP";
			String emailMessageBody = "<p>Hi there!</p><p>Your new OTP is " + phoneCode
					+ ". </p> <p>Payment GateWay Team </p>";
	...
	public int phoneVerification(MerchantModel merchantModel) throws Exception {
		...
		int result = merchantDao.phoneVerification(merchantModel.getMerchantId(),
				String.valueOf(merchantModel.getPhoneCode()));
		if (result > 0) {
			...
			try {
				...
				String emailMessageBody = "<p>Hi there!</p><p>We got a request to create a new Payment Gateway account with your mobile number *****"
						+ phoneNumber + " and email ID <a href=#>" + userEmail
						+ "</a>. </p> <p>Payment GateWay Team </p>";
	...
	public int forgotPassword(MerchantModel merchantModel, HttpServletRequest request) throws Exception {
		...
		if (userId > 0) {
			...
			try {
				...
				String emailMessageBody = "<p>Hi there!</p><p>We got a request to reset your password for your Payment Gateway account </p><p><a href="
						+ resetLink + ">Click here</a> to change your password. </p> <p>Payment GateWay Team </p>";
	...
	public int changePassword(MerchantModel merchantModel) throws Exception {
		...
		int result = merchantDao.changePassword(merchantModel.getMerchantId(), merchantModel.getPassword(),
				merchantModel.getConfirmPassword());
	...
	public int resend(MerchantModel merchantModel) throws Exception {
		...
		if (result > 0) {
			...
			try {
				...
				String header = "OTP";
				String emailMessageBody = "<p>Hi there!</p><p>Your new OTP is " + phoneCode
						+ ". </p> <p>. Thank you for using Lebupay. </p>";
				String subject = messageUtil.getBundle("new.otp.email.subject");
	...
	public int emailInvoicing(EmailInvoicingModel emailInvoicingModel) throws Exception {
		...
		try {
			...
			String emailMessageBody = "<p>Dear " + emailInvoicingModel.getFirstName() + " "
					+ emailInvoicingModel.getLastName() + "</p>" + "<p>Your Invoice No is "
					+ emailInvoicingModel.getInvoiceNo() + " for the type " + emailInvoicingModel.getType() + "</p>"
					+ " <p>Your Billing Amount is " + emailInvoicingModel.getBDT() + " "
					+ emailInvoicingModel.getAmount() + "</p>" + " <p>" + emailInvoicingModel.getDescription() + "</p>"
			...
			// sendMail.send(emailInvoicingModel.getEmailId(), messageBody,
			// emailInvoicingModel.getSubject());
			result = 1;
	...
	public HashMap<String, Object> link(EmailInvoicingModel emailInvoicingModel, HttpServletRequest request)
		...
		String customerDetails = "{\"firstName\": \"" + emailInvoicingModel.getFirstName() + "\",  \"lastName\": \""
				+ emailInvoicingModel.getLastName() + "\",  \"email\": \"" + emailInvoicingModel.getEmailId()
				+ "\",  \"mobileNumber\": \"" + emailInvoicingModel.getMobileNumber() + "\"}";
		...
		if (result1 > 0) {
			...
			if (transactionId > 0) {
				...
				String paymentLink = "<a href=" + basePath + "link-pay?SESSIONKEY="
						+ URLEncoder.encode(paymentInfo, "UTF-8") + ">Click here</a>";
				...
				try {
					...
					String bitlyFullUrl = bitlyBaseUrl + bitlyAccessToken + "&longUrl=" + baseLinkForBitly
							+ "&format=txt";
				...
				if (emailInvoicingModel.getPluggers().contains("email")) {
					...
					try {
						...
						String emailMessageBody = "<p>Dear " + emailInvoicingModel.getFirstName() + " "
								+ emailInvoicingModel.getLastName() + ",</p>" + "<p>Your Payment Link is: "
								+ paymentLink + "</p>" + " <p>" + emailInvoicingModel.getDescription() + "</p>"
								+ " <p>Payment GateWay Team </p>";
						...
						jsonReqString = jsonReqString.replace("replace_header_here", header);
						jsonReqString = jsonReqString.replace("replace_emailMessageBody_here", emailMessageBody);
						jsonReqString = jsonReqString.replace("replace_firstName_here",
								emailInvoicingModel.getFirstName());
						jsonReqString = jsonReqString.replace("replace_lastName_here",
								emailInvoicingModel.getLastName());
						jsonReqString = jsonReqString.replace("replace_paymentLink_here", paymentLink);
						jsonReqString = jsonReqString.replace("replace_description_here",
								emailInvoicingModel.getDescription());
				...
				if (emailInvoicingModel.getPluggers().contains("sms")) {
					...
					try {
						// Send SMS
						sendSMS.smsSend(emailInvoicingModel.getMobileNumber(),
								"Dear " + emailInvoicingModel.getFirstName() + " " + emailInvoicingModel.getLastName()
										+ ", Your Payment Link is: " + bitlyLink
	...
	public List<CardTypePercentageModel> getAllCardPercentageByMerchantId(Long merchantId) throws Exception {
		...
		List<CardTypePercentageModel> cardTypePercentageModels = merchantDao
				.getAllCardPercentageByMerchantId(merchantId);
