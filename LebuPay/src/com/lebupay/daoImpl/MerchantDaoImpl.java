...
/**
 * This is MerchantDaoImpl extends BaseDao and Implements MerchantDao Interface used to perform operation on Merchant Module.
 * @author Java Team
 *
 */
@Repository
public class MerchantDaoImpl extends BaseDao implements MerchantDao{
	...
	/**
	 * This method is used for checking whether the Mobile No is associated with other Merchant or not during update operation.
	 * @param mobileNo
	 * @param merchantId
	 * @return MerchantModel
	 * @throws Exception
	...
	/**
	 * This method is used to check whether the Email Id is already present in Database or not during sign up.
	 * @param emailId
	 * @return MerchantModel
	 * @throws Exception
	 */
	...
	/**
	 * This method is used for checking whether the Email Id is associated with other Merchant or not during update operation.
	 * @param emailId
	 * @param merchantId
	 * @return MerchantModel
	 * @throws Exception
	...
	/**
	 * This method is used for merchant signup.
	 * Insertion done into Merchant Master, Parameter Master & CheckOut Master Tables.
	 * @param merchantModel
	 * @return long
	 * @throws Exception
	 */
	public long merchantSignUp(MerchantModel merchantModel) throws Exception {
		...
		try {
			...
			String sql1 = "insert into MERCHANT_MASTER (MERCHANT_ID, TYPE_ID, FIRST_NAME, LAST_NAME, EMAIL_ID, MOBILE_NO, PASSWORD, UNIQUE_KEY, ACCESS_KEY, SECRET_KEY, PHONE_CODE, PHONE_VERIFIED, STATUS, CREATED_BY, CREATED_DATE) "
					+ "values(MERCHANT_MASTER_SEQ.nextval,"
					+ "(select TYPE_ID from type_master where type_name = '"+typeName+"'),"
					+ ":first_name,"
					+ ":last_name,"
					+ ":email_id,"
					+ ":mobile_no,"
					+ "( select standard_hash(:password, 'MD5') md5 from DUAL),"
					+ ":UNIQUE_KEY,"
					+ ":ACCESS_KEY,"
					+ ":SECRET_KEY,"
					+ ":phone_code,"
			...
			try { // Closing Connection Object
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Connection not closed for merchantSignUp"
							+ e.getMessage());
				}
	...
	/**
	 * This method is used for phone verification. If verification is done then the corresponding phone_verified field is updated
	 * @param merchantID
	 * @param phoneCode
	 * @return int
	 * @throws Exception
					...
					try {
						...
						connection1.commit();
							...
							try { // Closing Connection Object
								...
								if (logger.isDebugEnabled()) {
									logger.debug("Connection not closed for phoneValidation"
											+ e.getMessage());
								}
	...
	/**
	 * This method is used for Login Check Of the Merchant.
	 * Status active means only login and company profile can be add by Merchant.
	 * Status activate means Admin has given the permission to the Merchant for Performing Transaction.
	 * @param userName
	 * @param password
	 * @return MerchantModel
	 * @throws Exception
		...
		try {
			...
			if(userName.contains("@")) { // User has provided Email ID for login
				...
				sql = "select m.MERCHANT_ID," // 1
						...
						+ "from MERCHANT_MASTER m left outer join TYPE_MASTER t on m.TYPE_ID = t.TYPE_ID "
						+ "left outer join COMPANY_MASTER c on m.MERCHANT_ID = c.MERCHANT_ID "
						+ "where m.EMAIL_ID=:EMAIL_ID and m.PASSWORD =( select standard_hash(:PASSWORD, 'MD5') md5 from DUAL) and (m.STATUS =:STATUS1 OR m.STATUS =:STATUS2)";// and m.PHONE_VERIFIED =:PHONE_VERIFIED";
				...
				sql = "select m.MERCHANT_ID," // 1
						...
						+ "from MERCHANT_MASTER m left outer join TYPE_MASTER t on m.TYPE_ID = t.TYPE_ID "
						+ "left outer join COMPANY_MASTER c on m.MERCHANT_ID = c.MERCHANT_ID "
						+ "where m.MOBILE_NO =:MOBILE_NO and m.PASSWORD =:( select standard_hash(:PASSWORD, 'MD5') md5 from DUAL) and (m.STATUS =:STATUS1 OR m.STATUS =:STATUS2)";// and m.PHONE_VERIFIED =:PHONE_VERIFIED";
				...
				//pst.setIntAtName("PHONE_VERIFIED", Status.ACTIVE.ordinal()); // phone_verified
				ResultSet rs =  pst.executeQuery();
				if(rs.next()){
	...
	/**
	 * This method is used for insert the company details for the Merchant first time.
	 * @param companyModel
	 * @return long
	 * @throws Exception
	 */
	public long insertCompany(CompanyModel companyModel) throws Exception {
		...
		try {
			...
			String sql = "insert into COMPANY_MASTER (COMPANY_ID,MERCHANT_ID,COMPANY_NAME,DBA,SERVICE,YEARS_IN_BUSINESS,PHONE,FAX,WEBSITE,IP,ADDRESS,NET_WORTH,OTHER_BANK,"
					...
					+ "values(COMPANY_MASTER_SEQ.nextval,"
					+ ":MERCHANT_ID,"
					+ ":COMPANY_NAME,"
					+ ":DBA,"
					+ ":SERVICE,"
			...
			try { // Closing Connection Object
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Connection not closed for insertCompany"
							+ e.getMessage());
				}
	...
	/**
	 * This method is used to fetch the Active But Not Activated By Admin Merchant Details according to his/her ID.
	 * @param merchantId
	 * @return MerchantModel
	 * @throws Exception
	 */
	...
	/**
	 * This method is used to fetch the Active And Activated By Admin Merchant Details according to his/her ID.
	 * @param merchantId
	 * @return MerchantModel
	 * @throws Exception
	 */
	...
	/**
	 * This method is used for changing the old password to new one.
	 * First checking is done whether old password is valid or not, if valid then the new password is being updated to the password field.
	 * @param merchantID
	 * @param oldPassword
	 * @param newPassword
	 * @return int
		...
		try {
			...
			connection.commit();
				...
				try { // Closing Connection Object
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Connection not closed for skipCompany"
								+ e.getMessage());
					}
	...
	public int updateProfile(MerchantModel merchantModel) throws Exception {
		...
		try {
			...
			String sql = "update MERCHANT_MASTER set first_name =:first_name,"
					+ "last_name =:last_name,"
					+ "email_id =:email_id,"
					+ "mobile_no =:mobile_no,"
			...
			connection.commit();
				...
				try { // Closing Connection Object
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Connection not closed for updateProfile"
								+ e.getMessage());
					}
	...
	/**
	 * This method is used for resending the phone code. Also the latest generated code is been updated into the database.
	 * @param merchantModel
	 * @return int
	 * @throws Exception
	 */
	...
	private String getSearchQueryForActiveMerchant(DataTableModel dataTableModel, boolean isCount){
		...
		if(!isCount){
			sql = "select MERCHANT_ID,FIRST_NAME,LAST_NAME,EMAIL_ID,MOBILE_NO,STATUS,CREATED_DATE,CREATED_BY,MODIFIED_DATE,MODIFIED_BY "+sql;
		}
		else{
	...
	/**
	 * This method is used for fetch the Merchant List according to the searching criteria.
	 * @param dataTableModel
	 * @param status
	 * @return List<MerchantModel>
	 * @throws Exception
	...
	public List<MerchantModel> fetchActiveMerchantList(DataTableModel dataTableModel, Status status) throws Exception {
		...
		try {
			...
			String sql= getSearchQueryForActiveMerchant(dataTableModel, false)+" AND TYPE_ID=(select TYPE_ID from type_master where type_name = '"+typeName+"') AND STATUS !=:STATUS order by "+ dataTableModel.getOrderColumn()+" "+ dataTableModel.getOrderBy() +" OFFSET "+ dataTableModel.getStart() +" ROWS FETCH NEXT "+ dataTableModel.getLength() +" ROWS ONLY";
	...
	/**
	 * This method is used to get the total number of Merchants in the Database according to the AJAX Call.
	 * @param dataTableModel
	 * @param status
	 * @return int
	 * @throws Exception
	...
	public int getActiveMerchantListCount(DataTableModel dataTableModel, Status status) throws Exception {
		...
		try {
			...
			String sql= getSearchQueryForActiveMerchant(dataTableModel, true)+" AND  TYPE_ID=(select TYPE_ID from type_master where type_name = '"+typeName+"') AND STATUS !=:STATUS";
	 ...
	 * @throws Exception
	 */
	public List<MerchantModel> fetchAllActiveMerchantListForExport(String userType, DataTableModel dataTableModel, Status status) throws Exception {
		...
		try {
			...
			String sql= getSearchQueryForActiveMerchant(dataTableModel, false)+"  AND  TYPE_ID=(select TYPE_ID from type_master where type_name = '"+typeName+"') AND STATUS !=:STATUS order by "+ dataTableModel.getOrderColumn()+" "+ dataTableModel.getOrderBy();
	...
	public int activateMerchant(long merchantId, long adminId, Status status) throws Exception {
		...
		try {
			...
			String sql = "update MERCHANT_MASTER set STATUS =:STATUS,"
			...
			connection.commit();
				...
				try { // Closing Connection Object
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Connection not closed for activateMerchant"
								+ e.getMessage());
					}
	...
	/**
	 * This method is used by the Merchant to fetch the Company Details w.r.t Merchant ID.
	 * @param merchantId
	 * @return CompanyModel
	 * @throws Exception
	 */
	public CompanyModel fetchCompanyByMerchantId(long merchantId) throws Exception {
		...
		try {
				String sql = "select c.COMPANY_ID," // 1
						...
						+ "where c.MERCHANT_ID=:MERCHANT_ID and c.STATUS !=:STATUS";
	...
	public int updateCompany(CompanyModel companyModel) throws Exception {
		...
		OraclePreparedStatement pst = null;
		try {
			String sql = "update COMPANY_MASTER set COMPANY_NAME=:COMPANY_NAME,"
					+ "DBA=:DBA,"
					+ "SERVICE=:SERVICE,"
					...
					+ "PROJECT_NO_OF_TPM=:PROJECT_NO_OF_TPM,"
					+ "PROJECT_VOL_OF_TPM=:PROJECT_VOL_OF_TPM,"
					+ "MAX_AMT_ST=:MAX_AMT_ST,"
					+ "MAX_NO_TPD=:MAX_NO_TPD,"
					+ "MAX_VOL_TPD=:MAX_VOL_TPD,"
					+ "PAN_NO=:PAN_NO,"
					+ "NID=:NID,"
					+ "MODIFIED_BY=:MODIFIED_BY,"
					+ "MODIFIED_DATE=localtimestamp(0),"
					+ "MECHANDISING=:MECHANDISING,"
					+ "CONTACTPERSON=:CONTACTPERSON,"
					+ "DESIGNATION=:DESIGNATION,"
					+ "MOBILE=:MOBILE,"
					+ "EMAIL=:EMAIL,"
					+ "CODESEC=:CODESEC where COMPANY_ID =:COMPANY_ID";
	...
	/**
	 * This method is used to fetch the Active But Not Activated By Admin Merchant Details according to his/her ACCESSKEY.
	 * @param accessKey
	 * @return MerchantModel
	 * @throws Exception
	 */
	...
	public int createUserIdAndPassword(MerchantModel merchantModel) throws Exception {
		...
		try {
			...
			String sql = "update MERCHANT_MASTER set ebl_user_name =:ebl_user_name,"
			...
			connection.commit();
				...
				try { // Closing Connection Object
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Connection not closed for Merchant create UserId And Password By Admin"+ e.getMessage());
					}
	...
	public MerchantModel fetchMerchantByAccessKey(String accessKey) throws Exception {
		...
		try {
				...
				String sql1 = "select m.MERCHANT_ID," // 1
						...
						+ "from MERCHANT_MASTER m left outer join TYPE_MASTER t on m.TYPE_ID = t.TYPE_ID "
						+ "left outer join COMPANY_MASTER c on m.MERCHANT_ID = c.MERCHANT_ID "
						+ "where m.ACCESS_KEY='"+accessKey+"' and m.STATUS ='"+Status.ACTIVE.ordinal()+"'";
