...
/**
 * This class is used to all the methods that are not related to any session and
 * all error pages.
 * 
 * @author Java-Team
 *
...
public class IndexController extends BaseDao implements SaltTracker {
		...
		try {
			...
			if (Objects.isNull(quickPayModel))
				...
				try {
					...
					merchantModel = merchantService
							.fetchActiveMerchantById(quickPayModel.getMerchantModel().getMerchantId());
	...
	public String linkPayPage(HttpServletRequest httpServletRequest, Model model, @RequestParam String SESSIONKEY) {
		...
		try {
			...
			model.addAttribute("merchantName", transactionModel.getMerchantModel().getFirstName() + " "
					+ transactionModel.getMerchantModel().getLastName());
			...
			model.addAttribute("logo", transactionModel.getLogo());
			...
			try {
				...
				writeLogV2(0L, "IndexController/link-pay", 1, "initial txnId:" + transactionId + ",SESSIONKEY:"
						+ SESSIONKEY + ",transactionId1: " + transactionId1 + ",transactionId:" + transactionId);
			...
			// TODO Removed by
			/*
			 * model.addAttribute("citybankMerchantId",
			 * transactionModel.getMerchantModel().getCityMerchantId());
			 * 
			 * model.addAttribute("eBLUserName",
			 * transactionModel.getMerchantModel().getEblUserName());
			 * model.addAttribute("eBLUserPassword",
			 * transactionModel.getMerchantModel().getEblPassword());
			 * model.addAttribute("eBLUserId",
			 * transactionModel.getMerchantModel().getEblId()); //WASIF 20181115
			 * model.addAttribute("sEBLUserName",
			 * transactionModel.getMerchantModel().getSeblUserName());
			 * model.addAttribute("sEBLUserPassword",
			 * transactionModel.getMerchantModel().getSeblPassword());
			 * model.addAttribute("sEBLUserId",
			 * transactionModel.getMerchantModel().getSeblId());/
			 **/
