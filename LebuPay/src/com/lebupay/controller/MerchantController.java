...
public class MerchantController implements SaltTracker {
	...
	public @ResponseBody CommonModel merchantLogin(HttpServletRequest httpServletRequest,@RequestBody MerchantModel merchantModel) {
		...
		try {
			...
			if(merchantModel.getCreatedBy() == 0) { // No Company Skip
				...
				if(merchantModel.getCompanyModel().getCompanyId() != 0){
					...
					List<TransactionModel> transactionModels = transactionService.fetchAllTransactionByMerchantId(merchantSessionModel.getMerchantId());
	...
	public String listTransactionSearch(@ModelAttribute DataTableModel dataTableModel, 
		...
		try {
			...
			List<TransactionModel> transactionModels = transactionService.fetchAllTransactionsByMerchantIdNew(dataTableModel, merchantSessionModel.getMerchantId());
	 ...
	 */
	/*@RequestMapping(value = "/list-transaction", method = RequestMethod.POST)
	public @ResponseBody DataTableModel listTransaction(@ModelAttribute DataTableModel dataTableModel, 
		...
		try {
			MerchantSessionModel merchantSessionModel = (MerchantSessionModel) httpServletRequest.getSession().getAttribute("merchantModel");
			transactionService.fetchAllTransactionsByMerchantId(dataTableModel, merchantSessionModel.getMerchantId());
    ...
    public void downloadListDisburseMerchantExcel(HttpServletResponse httpServletResponse, 
		...
		try {
			MerchantSessionModel merchantSessionModel = (MerchantSessionModel) httpServletRequest.getSession().getAttribute("merchantModel");
			bookData = transactionService.exportAllTransactionsForExcelByMerchantId(dataTableModel, dataTableModel.getNoOfColumns(), merchantSessionModel.getMerchantId());
	...
	public String mailForgotPasswordPage(HttpServletRequest httpServletRequest,
		...
		try {
			...
			if (!org.springframework.util.CollectionUtils.isEmpty(activeSalts)& activeSalts.contains(salt)) {
				...
				if (result == 1) {
					...
					redirectAttributes.addFlashAttribute("merchant.password.wrong",messageUtil.getBundle("merchant.password.wrong"));
	...
	public String editMerchant(HttpServletRequest httpServletRequest,
			Model model) {
		if (logger.isInfoEnabled()) {
			...
			if (!org.springframework.util.CollectionUtils.isEmpty(activeSalts) & activeSalts.contains(salt)) {
				...
				try {
					...
					if (result > 0) {
						...
						redirectAttributes.addFlashAttribute("merchant.profile.update.success", messageUtil.getBundle("merchant.profile.update.success"));
					...
					for (Entry<String, Exception> entry : ((FormExceptions) e1).getExceptions().entrySet()) {
						if(entry.getKey().equals("common.invalid.phone.code") || entry.getKey().equals("common.phone.code.required")){
							
							MerchantSessionModel merchantSessionModel = (MerchantSessionModel)  httpServletRequest.getSession().getAttribute("merchantModel");
							MerchantModel merchant = merchantService.fetchActiveMerchantById(merchantSessionModel.getMerchantId());
					...
					e.printStackTrace();
					redirectAttributes.addFlashAttribute("merchant.profile.update.failed", messageUtil.getBundle("merchant.profile.update.failed"));
				}
			}
		} catch (Exception e) {
	...
	public String resend(Model model, HttpServletRequest httpServletRequest) {
		...
		try {
			...
			if(result > 0) {
				...
				model.addAttribute("common.phone.code.resend.successfully", messageUtil.getBundle("common.phone.code.resend.successfully"));
	...
	public String otpCheck(@ModelAttribute MerchantModel merchantModel,
			HttpServletRequest httpServletRequest, Model model) {
		...
		try {
			...
			if(result > 0) {
				...
				model.addAttribute("merchantId", merchantModel.getMerchantId());
				model.addAttribute("merchantModelProfile", merchantModel);
				model.addAttribute("common.phone.code.resend.successfully", messageUtil.getBundle("common.phone.code.successfully"));
				return "merchant-otp-check";
	...
	public String createCompanyInfo(@ModelAttribute CompanyModel companyModel,
		...
		try {
			...
			if (!org.springframework.util.CollectionUtils.isEmpty(activeSalts)& activeSalts.contains(salt)) {
				...
				if (result > 0) {
					...
					redirectAttributes.addFlashAttribute("merchant.company.successfully", messageUtil.getBundle("merchant.company.successfully"));
					return "redirect:/merchant/dashboard";
	...
	public void downloadExcel(HttpServletResponse httpServletResponse,@ModelAttribute DataTableModel dataTableModel,
		...
		try {
			MerchantSessionModel merchantSessionModel = (MerchantSessionModel) httpServletRequest.getSession().getAttribute("merchantModel");
			bookData = merchantService.exportTicketsForExcel(dataTableModel,dataTableModel.getNoOfColumns(),merchantSessionModel.getMerchantId());
		} catch (Exception e) {
		...
		String headerValue = String.format(
				"attachment; filename=\"marchentTicket.xls\"", "excel file");
		httpServletResponse.setHeader(headerKey, headerValue);
	...
	public String checkout(HttpServletRequest request, Model model) {
		...
		try {
			model.addAttribute("parameterModels", checkOutService.fetchParametersById(merchantSessionModel.getMerchantId()));
		} catch (Exception e) {
		...
		try {
			model.addAttribute("checkoutModel", checkOutService.fetchCheckoutById(merchantSessionModel.getMerchantId()));
		} catch (Exception e) {
	...
	public String restoreDefaultLayout(HttpServletRequest httpServletRequest,Model model,
		...
		try {
			...
			if (result > 0) {
				...
				redirectAttributes.addFlashAttribute("restore.successfully", messageUtil.getBundle("restore.successfully"));
	...
	public String addCheckoutParameter(HttpServletRequest httpServletRequest,@ModelAttribute ParameterModel parameterModel,
			RedirectAttributes redirectAttributes) {
		if (logger.isInfoEnabled()) {
		...
		try {
			...
			if (!org.springframework.util.CollectionUtils.isEmpty(activeSalts)& activeSalts.contains(salt)) {
				try {
					...
					if(result > 0)
						redirectAttributes.addFlashAttribute("invoice.send.success", messageUtil.getBundle("invoice.send.success"));
					else
						redirectAttributes.addFlashAttribute("invoice.send.failed", messageUtil.getBundle("invoice.send.failed"));
	...
	public String uploadQuickpayFile(HttpServletRequest request,@RequestParam("file") MultipartFile file, 
		...
		try {
			...
			if(Objects.nonNull(type)) {
				if(!type.equals("csv")){
					redirectAttributes.addFlashAttribute("csv.format.mismatch", messageUtil.getBundle("csv.format.mismatch"));
				}
			} else{
		...
		String fileName = new Date().getTime() + "1_"
				+ file.getOriginalFilename();
		String fileName1 = new Date().getTime() + "2_"
				+ file.getOriginalFilename();
					...
					if (j == 1) { // First Row
						...
						if (!Util.isEmpty(columns[3])) {
							...
							CSVUtils.writeLine(writer, Arrays.asList(
									quickPayModel.getProductSKU(),
									quickPayModel.getProductName(),
									quickPayModel.getOrderAmount(),
									quickPayModel.getCustom1(), "Link"));
							...
							CSVUtils.writeLine(writer, Arrays.asList(
									...
									quickPayModel.getOrderAmount(),
									quickPayModel.getCustom1(),
									quickPayModel.getCustom2(),
									quickPayModel.getCustom3(), "Link"));
							...
							CSVUtils.writeLine(writer, Arrays.asList(
									quickPayModel.getProductSKU(),
									quickPayModel.getProductName(),
									quickPayModel.getOrderAmount(), "Link"));
						}
		...
		try {
			...
			if(merchantModel.getCreatedBy() == 0) { // No Company Skip
				...
				if(merchantModel.getCompanyModel().getCompanyId() != 0){
					...
					model.addAttribute("transactionAmount", merchantModel.getTransactionAmount());
					model.addAttribute("loyaltyPoint", merchantModel.getLoyaltyPoint());
					model.addAttribute("companyModel", merchantService.fetchCompanyByMerchantId(merchantSessionModel.getMerchantId()));
					model.addAttribute("merchantId", merchantModel.getMerchantId());
	...
	public String editConpanyDetails(@ModelAttribute CompanyModel companyModel, 
			HttpServletRequest httpServletRequest, 
			RedirectAttributes redirectAttributes) {
		if (logger.isInfoEnabled()) {
						...
						if(transactionId.equalsIgnoreCase("ALL")){
							...
							for(TransactionModel transactionModel:transactionModels){
								...
								if(transactionModel.getTransactionStatus() == 0){
									int result = transactionService.updateTransaction(merchantSessionModel.getMerchantId(), 1, transactionModel.getTransactionId());
								}
							}
							...
							for(String tId:transactionIds){
								int result = transactionService.updateTransaction(merchantSessionModel.getMerchantId(), 1, Long.parseLong(tId));
							}
							
						}
						
					}else{
						throw new FormExceptions(Arrays.asList(messageUtil.getBundle("merchant.transactionid.required")));
					}
			...
			if (!org.springframework.util.CollectionUtils.isEmpty(activeSalts)& activeSalts.contains(salt)) {
				...
				try {
					...
					if(Integer.parseInt(result.get("result").toString()) > 0 && result.containsKey("plugger")) {
						...
						redirectAttributes.addFlashAttribute("link.send.success", messageUtil.getBundle("link.send.success"));
					...
					e.printStackTrace();
					redirectAttributes.addFlashAttribute("invoice.send.failed", messageUtil.getBundle("link.send.failed"));
				}
	...
	public String cardPercentagePage(HttpServletRequest httpServletRequest, Model model) {
		...
		try {
			...
			MerchantSessionModel merchantSessionModel = (MerchantSessionModel) httpServletRequest.getSession().getAttribute("merchantModel");
			List<CardTypePercentageModel> cardTypePercentageModels = merchantService.getAllCardPercentageByMerchantId(merchantSessionModel.getMerchantId());
